...
export default class Structured extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
  ...
  boxPosition = x => {
    ...
    return this.state.scrollY.interpolate({
      inputRange: [0, height],
      outputRange: [startPos + BOX_MAX_HEIGHT * c, -3 * height],
      extrapolate: 'extend',
  ...
  boxSize = (x, DIMENSION_SIZE) => {
    ...
    return this.state.scrollY.interpolate({
      inputRange: [0, height],
      outputRange: [DIMENSION_SIZE * c1, DIMENSION_SIZE * y2 * -1],
      extrapolate: 'extend',
      easing: Easing.linear,
    });
  };
